...
horn_bolt_n = 2;    // 2, at 0/180 deg - ALONG the rod, so the offset rod
                    // passes to one side of both of them
...
/* ---------- PIVOT / SWEEP ------------------------------------------ */
pivot_x     = -8;   // moved in: the wider shell must stay on the plate
pivot_y     = 0;
...
slab_h   = 11;
hub_d    = 18;
hub_up   = 8;       // back to short: the rod is now OFFSET sideways, so it
                    // no longer passes under the horn hardware at all
rod_y_off = 9;      // rod axis offset from the pivot. Must clear the driver
                    // bore (+-2.8) and the screw-head pocket (+-3.5).
                    // 9 gives 2.9 and 2.2 mm respectively. Minimum ~8.
...
/* ---------- PEDESTAL ----------------------------------------------- */
r_in     = 23;      // widened for the offset rod's inner nut (swings to r=21.5)
r_out    = 27;      // pivot_x=-8 keeps this just on the 70mm plate
r_plat   = 29;
plat_th  = 7;      // no ribs, and the bridge span is now 46mm
win_pad  = 12;      // extra margin beyond the computed arm/bob extent
wall     = 2.4;
clr      = 0.4;
...
// the bob sits off the rod axis, so its swing radius is the hypotenuse
bob_r     = sqrt(L*L + rod_y_off*rod_y_off);
bob_out_r = sqrt(pow(L+bob_len/2,2) + rod_y_off*rod_y_off);
bob_ang   = atan2(rod_y_off, L);   // bob leads the rod axis by this much
chord   = 2*bob_r*sin(sweep_deg/2);
rod_m   = 0.17*(L + bob_len/2 + 20);
M_total = robot_mass + print_mass + bob_mass + rod_m;
dx_com  = bob_mass*chord/M_total;

// window must clear the arm body AND the bob, both of which are offset
arm_lo = atan2(-slab_w/2, slab_r1);
arm_hi = max(atan2(rod_y_off+slab_w/2, slab_r1),
             atan2(rod_y_off+bob_od/2, L-bob_len/2));
win_a0 = sweep_start + arm_lo - win_pad;
win_a1 = sweep_start + sweep_deg + arm_hi + win_pad;

// shield: auto-size to the bob's swept path unless overridden
sh_r  = shield_r  > 0 ? shield_r  : bob_out_r + 4;
...
inner_nut_r = sqrt(pow(slab_in+nut6_th,2) + pow(rod_y_off+nut6_af_c/2,2));
// vertical clearance between the rod passing through and the horn
// hardware sitting above it
echo(str(">>> rod offset ", rod_y_off, " mm: spans y ", rod_y_off-rod_bore/2,
         " .. ", rod_y_off+rod_bore/2));
echo(str("    clears driver bore by ", rod_y_off-rod_bore/2-(shaft_d+0.6)/2,
         " mm, screw-head pocket by ", rod_y_off-rod_bore/2-shaft_screw_head_d/2, " mm"));
if (rod_y_off-rod_bore/2-shaft_screw_head_d/2 < 1)
   echo("!!! ROD FOULS THE HORN SCREW HEAD - increase rod_y_off");
echo(str(">>> bob leads the rod axis by ", bob_ang,
         " deg - offset your servo zero by this to centre the sweep"));
...
module arm(){
   ...
   difference(){
      union(){
         ...
         // slab is wide enough to cover BOTH the hub and the offset rod
         translate([-slab_in, -slab_w/2, z0])
            cube([slab_in+slab_r1, slab_w+rod_y_off, slab_h]);
      }
      ...
      translate([-slab_in-2, rod_y_off, z_rod]) rotate([0,90,0])
...
module arm_and_rod(){
   color("khaki") arm();
   color("goldenrod") translate([-slab_in-6, rod_y_off, z_rod]) rotate([0,90,0])
      cylinder(d=6, h=slab_in+L+bob_len/2+8);
   color("dimgray") translate([L, rod_y_off, z_rod]) rotate([0,90,0])
...
/* =====================================================================
   ...
   ASSEMBLY  (order is now FREE - this is why the rod is offset)
     Because the rod passes to one SIDE of the pivot, it no longer crosses
     the vertical driver bore. You can do the horn screws and the central
     shaft screw with the rod already fitted, or fit the rod last. Either
     way round works, which is the whole point of the offset.

     1. BENCH: horn into the arm's top pocket, 2 horn screws driven DOWN
        into the blind holes. They sit at 0/180 deg, along the rod, so the
        rod clears both. Arm + horn is one piece.
     2. BENCH: rod through the arm, nut on the inner face and nut + washer
        on the outer face. Slide to set arm length, then tighten.
     3. Centre the servo in software, allowing for the bob_ang offset the
        echo reports (the bob leads the rod axis by ~10 deg).
     4. Servo down through the platform hole, tabs bolted from above.
     5. Arm assembly in through the WINDOW, lift onto the shaft. The rod
        gives you something to hold and rotate while finding the spline.
     6. Central shaft screw UP through the plate access hole and the arm's
        vertical bore. The rod is offset, so it is not in the way.
     7. Washer stack on the outer rod end between two nuts, locknut out.
   ...
   The central screw stays reachable for re-tightening later without
   removing the rod - another consequence of the offset.
