...
\end{align}
paid once at the switch, whereas $\epsilon_k$ is a rate incurred afresh every step the incumbent is held; a threshold on $\epsilon_k$ itself thus compares a rate to a one-time cost, and is either tripped by transients or, set honestly, silent until the robot is already failing.
...
The slack $\kappa$ keeps its usual role of fixing the in-control average run length and is the only quantity still calibrated by hand.
...
At the operating point of Section~\ref{sec:exp-payload}, $h=5$ prices a switch at $0.13$\,nats under the measured $s = 0.025$\,nats -- a fifth of a prior standard deviation -- whereas the gaits the agent actually applies lie some twelve standard deviations out; pricing that realized move would delay the trigger by a median of $10.8$\,s and silence it in most bouts.
The discrepancy reflects that the realized move is large because the search explores, not because correction demands it; pricing the correction the belief itself deems necessary, via its control gain, would close this gap, which we leave to future work.
Calibration in the experiments below therefore rests on $\kappa$: over the $150$\,s of settled nominal walking the standardized excess never exceeds $2.7$, so at $\kappa = 3$ every increment is negative, the accumulator cannot leave zero, and no false alarm is possible whatever $h$ is; $h$ only imposes a short persistence requirement once a perturbation lifts the excess past the slack.
(The transient following a recovery reaches several hundred standard deviations and is instead kept from firing by the arming logic of Section~\ref{sec:trigger}.)

What the slack buys can be stated precisely.
Writing $e_k = (\epsilon_k - \bar{\epsilon})/s$ and $T = \inf\{k : S_k > h\}$ for the firing time, the accumulator admits both a deterministic and an exponential guarantee against false alarms.
...
Part \emph{(i)} governs the experiments: the observed excess peaks at $2.69$ over the $150$\,s baseline, below $\kappa=3$, so no false alarm is possible on that sample regardless of $h$.
Part \emph{(ii)} covers excursions beyond the observed support: for Gaussian $e_k$, $\theta^\star = 2(\kappa-\mu)/\sigma^2$ gives $e^{-2\kappa h} = 9\times10^{-14}$ at the standardized values, though this is optimistic since $\epsilon_k$ is strongly autocorrelated at $100$\,Hz (lag-one correlation $0.92$, long-run variance $8.9\times$ the marginal); substituting the long-run variance for the one-step generating function instead gives $\theta^\star \approx 2.7$ and a still-serviceable bound below $10^{-6}$.
Both figures are conditional on $(\bar\epsilon,s)$, themselves estimated from a $125$-sample warm-up with an integrated autocorrelation time of about $33$, i.e.\ roughly four effective samples.

No matching guarantee is claimed against missed detections: it would require the post-change increments to have positive mean, whereas the payload shift of Section~\ref{sec:exp-payload} raises the mean standardized excess only to $0.93$ against $\kappa=3$, so the accumulator still drifts downward on average and fires through excursions of a heavy-tailed statistic rather than through drift.
This suffices in practice -- every event in Section~\ref{sec:exp-payload} is detected -- but we treat the false-negative rate as an empirical matter throughout.
...
The second is where the danger lies: grid search, Bayesian optimization, and extremum seeking can judge a candidate gait only by walking on it, so on an already-perturbed robot a poor candidate does not merely score badly, it tips the robot over -- extremum seeking makes the point most sharply, its dither a deliberate perturbation of the loaded robot that costs more falls than the two search methods it is cheaper than.
Our agent instead screens each candidate against a learned map with an explicit memory of past falls, avoiding these destabilizing trials by making small, model-guided corrections rather than a faster search toward a distant optimum.
...
The ablation narrows this claim: a safety-filtered Gaussian-process agent, given the same fall memory but neither the unified goal-prior trigger nor the dynamics belief, matches our agent's fall rate, so it is the screening that pays, not the unified objective.
We therefore read the unified formulation as an architectural economy -- one goal prior specifying when to act and what to do, removing a separately tuned detector at no cost in falls here and detecting the shift somewhat sooner -- rather than as a performance result; whether that economy earns more where a detector's threshold is harder to hand-tune remains untested.
...
Staying upright and making progress are thus in tension here, and our agent, like the others, resolves it toward staying upright -- a clean comparison since all arms pay the same price, but one that leaves adapting \emph{without} surrendering the locomotion objective open, likely requiring a richer parameterization rather than a better search within this one.
A richer control representation, or a model expressive enough to reach the distant optimum without destabilizing exploration, is the central open problem; validating on hardware, and across a wider range of perturbations, are natural next steps.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\appendix
\section{Proof sketch of Proposition~\ref{prop:fpr}}
\label{appx:fpr}

Write $X_k = e_k - \kappa$ and $W_n = \sum_{k=1}^{n} X_k$ (with $W_0=0$), so Eq.~\ref{eq:trigger-cusum} reads $S_n = \max(0, S_{n-1} + X_n)$, $S_0=0$.
\emph{(i)} If $X_k \leq 0$ for every $k \leq n$, induction gives $S_k = 0$ for every $k \leq n$ (it holds at $k=0$, and $S_{k-1}=0 \Rightarrow S_k = \max(0,X_k)=0$), hence $T>n$ for any $h>0$: the accumulator cannot leave the origin on a stretch whose excess never exceeds the slack.
\emph{(ii)} Unrolling the recursion gives the Lindley representation $S_n = \max_{0 \leq j \leq n}(W_n - W_j)$; since $(X_1,\dots,X_n)$ and its reversal share a distribution, $S_n$ has the law of $\max_{0\leq m\leq n} W_m$ for each $n$, so $\sup_n S_n \overset{d}{=} M := \sup_n W_n$ and $\{T<\infty\} = \{M>h\}$.
Let $\theta^{\star}>0$ solve $\E[e^{\theta^{\star} X_k}]=1$.
Then $Z_n = \exp(\theta^{\star} W_n)$ is a non-negative martingale with $Z_0=1$; applying optional stopping at the bounded time $\tau \wedge n$, with $\tau = \inf\{n : W_n > h\}$, gives $1 = \E[Z_{\tau \wedge n}] \geq \E[Z_\tau \mathbf{1}\{\tau\leq n\}] \geq e^{\theta^{\star} h}\Pr(\tau \leq n)$.
Letting $n \to \infty$ and using $\Pr(\tau \leq n) \uparrow \Pr(M>h)$ yields $\Pr(T<\infty) = \Pr(M>h) \leq e^{-\theta^{\star} h}$, Eq.~\ref{eq:lundberg}. \hfill$\blacksquare$

For Gaussian increments $e_k \sim \cN(\mu,\sigma^2)$, $\theta(\mu-\kappa) + \theta^2\sigma^2/2 = 0$ gives $\theta^{\star} = 2(\kappa-\mu)/\sigma^2$, or $\theta^\star = 2\kappa$ and bound $e^{-2\kappa h}$ on standardized values.
When the $X_k$ are dependent, the same martingale argument applies to the limiting scaled cumulant generating function $\Lambda(\theta) = \lim_{n\to\infty} n^{-1}\ln\E[e^{\theta W_n}]$ in place of the one-step one: for stationary sequences under geometric mixing, $\Lambda$ is finite near the origin and $\Pr(T<\infty) \leq C\,e^{-\theta^{\star} h}$ with $\Lambda(\theta^{\star})=0$ and a constant $C\geq1$ set by the mixing rate, the exponent degrading roughly with the ratio of long-run to marginal variance (the substitution used in Section~\ref{sec:threshold}).
