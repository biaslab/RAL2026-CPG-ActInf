...
SWITCH_RAMP_STEPS = 40    # parameter ramp length (0.4 s), as in demo-speedbump
SLOPE_DEG = 10.0
N_COLS = 1600             # heightfield forward extent: +/- 40 m
FAR_SLOPE_Y = 60.0        # slope start outside the grid -> effectively flat
JITTER_STD = 0.002        # initial joint-angle jitter [rad]
TARGET_VX = 0.5           # v* of the shared velocity-tracking objective [m/s]
ROBOT_MASS = 10.0
G = 9.81
...
def _reset_with_jitter(p, robot, seed):
    """MARXEFE-style reset + 1 s settle, with a seeded jitter on the initial
...
def run_episode(terrain_cfg, seed, params_start, params_target=None,
                switch_step=None, duration=T_TOTAL):
    """One episode on `terrain_cfg`. Holds `params_start`; if `params_target`
    is given, ramps to it over SWITCH_RAMP_STEPS starting at `switch_step`.
